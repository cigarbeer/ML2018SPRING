...
if __name__ == '__main__':
    ...
    train(model=wv_rnn_model, X=idseqpad, y=label, batch_size=st.BATCH_SIZE, epochs=st.EPOCHS, validation_split=st.VALIDATION_SPLIT, save_model_path=st.SEMISUPERVISED_RNN_MODEL_CHECKPOINT_PATH)
    ...
    for i in range(1): 
        ...
        train(model=hw5.load_semisupervised_rnn_model(), X=idseqpad_cat, y=label_cat, batch_size=st.BATCH_SIZE, epochs=st.EPOCHS, validation_split=st.VALIDATION_SPLIT, save_model_path=st.SEMISUPERVISED_RNN_MODEL_CHECKPOINT_PATH)
    print('[done]') 
