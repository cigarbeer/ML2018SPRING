...
def net(input_shape, output_shape):
    ...
    model = nn_block(model, units=FULLY_CONNECTED_UNIT_NUM, n_layers=2, dropout_rate=0.5) 
...
def fit_generator(model, X, y, epochs, batch_size, model_saving_path):
    callbacks = [
        ModelCheckpoint(model_saving_path+'best.hdf5', monitor='val_acc', save_best_only=True, verbose=1), 
        # ModelCheckpoint(model_saving_path+'weights.{epoch:02d}-{val_loss:.4f}-{val_acc:.4f}.hdf5', monitor='val_loss', verbose=1), 
        EarlyStopping(monitor='val_acc', min_delta=1e-4, patience=50, verbose=1)
    ]
