...
def net(input_shape, output_shape):
    ...
    model = cnn_block(model, filters=256, kernel_size=(3, 3), n_layers=2, dropout_rate=0.25) 

    model.add(Flatten())

    model = nn_block(model, units=FULLY_CONNECTED_UNIT_NUM, n_layers=1, dropout_rate=0.25) 
    # model = nn_block(model, units=FLATTEN_IMAGE_SIZE, n_layers=2, dropout_rate=0.5) 
...
if __name__ == '__main__': 
    ...
    model = fit_generator(model, X, y, epochs=130, batch_size=BATCH_SIZE, model_saving_path='./')
