...
if __name__ == '__main__':
    ...
    hw5.get_tokenizer_with_punctuation(texts) 
    print('[save tokenizer]') 
    save_object(hw5, st.HW5_MODEL_PATH)  
    print('[translate texts to idx sequence]') 
    idseq = texts2idseq(ltexts, hw5.get_tokenizer_with_punctuation()) 
    ...
    wv_rnn_model = wordvector_rnn_classifier(hw5.get_wordvector(), hw5.get_tokenizer_with_punctuation(), hw5.max_document_size) 
    print('[start training]')
    train(model=wv_rnn_model, X=idseqpad, y=label, batch_size=st.BATCH_SIZE, epochs=st.EPOCHS, validation_split=st.VALIDATION_SPLIT, save_model_path=st.PUNCTUATION_MODEL_CHECKPOINT_PATH)
    print('[done]') 
